...
def plot_curves(results_dir, epochs, train_losses, valid_accs):
    ...
    plt.tight_layout()
    save_path = os.path.join(results_dir, 'training_curves_product600.png')
...
def save_summary(results_dir, best_valid_acc, test_acc):
    """将最终结果摘要保存到文本文件"""
    os.makedirs(results_dir, exist_ok=True)
    summary_path = os.path.join(results_dir, 'product_results.txt')
...
def main():
    ...
    data_dir = '/home/zhouruiqi/project/HW2/MLP/dataset/product'
    results_dir = '/home/zhouruiqi/project/HW2/MLP/results'
    model_results_dir = os.path.join(results_dir, 'model_weights')
    best_model_path = os.path.join(model_results_dir, 'product_mlp.pth')
